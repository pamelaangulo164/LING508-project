...
def test_meaning():
    lesion = EnglishTerm(term="lesion", pos=PartOfSpeech.NOUN)
    m1 = Meaning(
        description="Injury, trauma (e.g. bruise, fracture)",
        english_term=lesion,
    )
...
def test_examples_attach():
    ...
    ex_en = Example(
        language="en",
        text="The MRI showed a suspicious brain lesion.",
        meaning=m1,
    )
    ex_es = Example(
        language="es",
        text="La resonancia mostró una lesión sospechosa en el cerebro.",
        meaning=m1,
    )
